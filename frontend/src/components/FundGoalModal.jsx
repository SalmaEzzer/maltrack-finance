...
export default function FundGoalModal({
  ...
  useEffect(() => {
    ...
    setWalletId(
      goal?.walletId?._id ||
        goal?.walletId ||
        wallets.find((wallet) => wallet.isDefault)?._id ||
        wallets[0]?._id ||
        ""
    );
  }, [open, goal, wallets]);
  ...
  const p = useMemo(
    () => pct(goal?.currentAmount, goal?.targetAmount),
    [goal]
  );
  ...
  const selectedWallet =
    wallets.find((wallet) => wallet._id === walletId) || goal.walletId;
              ...
              <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                ...
                {goal.walletId ? (
                  <div className="mt-1 text-xs text-white/45">
                    Portefeuille :{" "}
                    {goal.walletId.name || selectedWallet?.name || "Wallet"}
                  </div>
                ) : null}
